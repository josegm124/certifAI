...
export default function Results() {
  ...
  return <main className="wrap wrap-wide">
    <div className="dash-top"><div><div className="eyebrow">Official result · {org}</div><h1 className="h1" style={{ fontSize: 32 }}>{aiSystemName}</h1></div><div className="no-print" style={{ display: "flex", gap: 10 }}>{server.badge && <button className="btn btn-primary" onClick={() => window.print()}>Print certificate</button>}<Link to="/dashboard" className="btn btn-ghost">Dashboard</Link></div></div>
    <div className="res-hero"><ScoreDial pct={server.result.overallScore} color={LEVEL_COLOR[level.id]} /><div className="lvlpanel" style={{ border: 0 }}><LevelBadge level={level.id} size={62} /><div className="lvlpanel-body"><div className="lvlpanel-name">{level.name}</div><div className="lvlpanel-state">{tier === 1 ? "Free readiness result · badge preview only" : "Backend score of record · finalized"}</div><div className="lvlpanel-blurb">{level.blurb}</div></div></div></div>
    ...
    {server.badge ? <div className="banner banner-info" style={{ marginTop: 14 }}><Check color={C.ocean} /><div><strong>Public badge issued · {server.badge.tier}</strong><br />Token <code>{server.badge.verificationToken}</code> · <a href={`http://localhost:3001/verify/${server.badge.verificationToken}`} target="_blank" rel="noreferrer">Verify badge</a></div></div> : tier === 1 ? <div className="banner banner-info" style={{ marginTop: 14 }}>This is a free readiness result. The badge image shows your eligible level as a preview; no public badge or certificate has been issued.</div> : <div className="banner banner-cap" style={{ marginTop: 14 }}>No public badge was issued for this result.</div>}
    {tier === 1 && assessmentId && <CertificateOptions assessmentId={assessmentId} levelId={level.id} />}
