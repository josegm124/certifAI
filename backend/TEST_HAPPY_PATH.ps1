...
Write-Host "[1/9] Creating Organization..." -ForegroundColor Green
...
Write-Host "   [OK] Name: $($org.name)" -ForegroundColor Green
Write-Host "   [OK] Tier: $($org.tier)" -ForegroundColor Green
Write-Host ""
...
Write-Host "   [OK] Assessment 1 ID: $assessId1" -ForegroundColor Green
Write-Host "   [OK] Tier: $($assess1.tier)" -ForegroundColor Green
Write-Host ""

# ========== STEP 3: Record 32 Answers (Tier 1) ==========
Write-Host "[3/9]  Recording 32 Answers (Tier 1)..." -ForegroundColor Green
$answerCount = 0
for ($i = 1; $i -le 32; $i++) {
  ...
  if ($i % 8 -eq 0) {
    Write-Host "   [OK] Saved $i/32 answers" -ForegroundColor Green
  }
}
Write-Host "   [OK] Total answers saved: $answerCount/32" -ForegroundColor Green
Write-Host ""

# ========== STEP 4: Compute Scores (Tier 1) ==========
Write-Host "[4/9]  Computing Scores (Tier 1)..." -ForegroundColor Green
...
Write-Host "   [OK] Overall Score: $($scoring1.overallScore)/5" -ForegroundColor Green
Write-Host "   [OK] Badge Tier: $($scoring1.badgeTier)" -ForegroundColor Green
Write-Host "   [OK] Completion: $($scoring1.completion.percentage)%" -ForegroundColor Green
Write-Host "   [OK] Critical Gating: $($scoring1.criticalGating)" -ForegroundColor Green
Write-Host "   [OK] Gaps Found: $($scoring1.gaps.Count)" -ForegroundColor Green
Write-Host ""

# ========== STEP 5: Upgrade to Tier 2 ==========
Write-Host "[5/9]  Upgrading to Tier 2 (Professional)..." -ForegroundColor Green
...
Write-Host "   [OK] Assessment 2 ID (Professional): $assessId2" -ForegroundColor Green
Write-Host "   [OK] Tier: $($assess2.tier)" -ForegroundColor Green
Write-Host ""

# ========== STEP 6: Copy Answers to Tier 2 ==========
Write-Host "[6/9]  Copying 32 Answers to Tier 2..." -ForegroundColor Green
$copyCount = 0
for ($i = 1; $i -le 32; $i++) {
  $score = (3 + ($i % 3))
...
Write-Host "   [OK] Copied $copyCount/32 answers to Tier 2" -ForegroundColor Green
Write-Host ""

# ========== STEP 7: Compute Scores (Tier 2) ==========
Write-Host "[7/9]  Computing Scores (Tier 2)..." -ForegroundColor Green
...
Write-Host "   [OK] Overall Score: $($scoring2.overallScore)/5" -ForegroundColor Green
Write-Host "   [OK] Badge Tier: $($scoring2.badgeTier)" -ForegroundColor Green
Write-Host "   [OK] Completion: $($scoring2.completion.percentage)%" -ForegroundColor Green
Write-Host ""

# ========== STEP 8: Issue Badge ==========
Write-Host "[8/9]  Issuing Badge..." -ForegroundColor Green
...
Write-Host "   [OK] Issued: $($badge.issuedAt)" -ForegroundColor Green
Write-Host "   [OK] Expires: $($badge.expiresAt)" -ForegroundColor Green
Write-Host "   [OK] Verification Token: $($badge.verificationToken.Substring(0, 12))..." -ForegroundColor Green
Write-Host ""
...
Write-Host "   [OK] Tier: $($verifyBadge.tier)" -ForegroundColor Green
Write-Host "   [OK] Expired: $(if ($verifyRes.StatusCode -eq 200) { 'No [OK]' } else { 'Yes' })" -ForegroundColor Green
Write-Host ""

# ========== SUMMARY ==========
Write-Host "====== TEST SUMMARY ======" -ForegroundColor Cyan
Write-Host ""
Write-Host "[PASS] Organization: $orgName ($orgId)" -ForegroundColor Green
Write-Host "[PASS] Tier 1 Assessment: $assessId1 (free)" -ForegroundColor Green
Write-Host "[PASS] Tier 2 Assessment: $assessId2 (professional)" -ForegroundColor Green
Write-Host "[PASS] Answers Recorded: 32 per assessment = 64 total" -ForegroundColor Green
Write-Host "[PASS] Scores Computed: Both tiers" -ForegroundColor Green
Write-Host "[PASS] Badge Issued: $($badge.tier) (expires $(([DateTime]$badge.expiresAt).ToString('yyyy-MM-dd')))" -ForegroundColor Green
Write-Host "[PASS] Badge Verified: Token valid, public accessible" -ForegroundColor Green
Write-Host ""
Write-Host "[LINK] PUBLIC SHARE LINK (no login needed):" -ForegroundColor Yellow
Write-Host "http://localhost:5173?verify=$($badge.verificationToken)" -ForegroundColor Yellow
Write-Host ""
Write-Host "[PASS] HAPPY PATH TEST PASSED" -ForegroundColor Green
Write-Host ""
